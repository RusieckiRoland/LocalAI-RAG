...
Persist HistorySummary (and a pointer/version).
In subsequent turns:
   - inject HistorySummary + **only the newest raw turns since the summary**.
If the injected history exceeds the budget again:
...
Run FAISS with a widened pool (example policy):
   - widen = max(50, top_k * 3) (exact formula is an implementation detail).
Compute keyword score on candidates (no extra index).
Blend scores (example):
   - final = alpha * semantic_score + beta * keyword_score_norm
Sort by final, trim to top_k.
...
Run Semantic (FAISS) → ranked list S.
Run BM25 → ranked list B.
Fuse with RRF:
...
csharp:MyApp.InvoiceController.SomeAction|METHOD**Output:**EntryNodes
...
edges restricted to graph_edge_allowlist**Output:**GraphNeighborhood (subgraph relevant to the question)
...
increment turn_loop_counter
if turn_loop_counter > max_turn_loops:
...
Use retrieval to find **entry points**.
Pull the dependency tree via the graph **right after retrieval**.
Use targeted lookups to fetch missing bodies/snippets.
If entry points remain weak after 1–2 attempts, stop and be explicit (no hallucinations).
Always enforce max_turn_loops.
