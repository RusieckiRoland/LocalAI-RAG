...
def test_app_config(mock_server) -> None:
    port = mock_server
    data = _get_json(f"http://127.0.0.1:{port}/app-config")
...
def test_search_and_query(mock_server) -> None:
    port = mock_server
    payload = {"consultant": "rejewski", "query": "hi", "enableTrace": True}
    out = _post_json(f"http://127.0.0.1:{port}/search", payload)
    assert "session_id" in out
    assert "results" in out
    assert "pipeline_run_id" in out

    out2 = _post_json(f"http://127.0.0.1:{port}/query", payload)
...
def test_pipeline_stream_and_cancel(mock_server) -> None:
    ...
    with request.urlopen(
        f"http://127.0.0.1:{port}/pipeline/stream?run_id=abc", timeout=2
    ) as resp:
        body = resp.read().decode("utf-8")
        assert "data:" in body
        assert "\"done\"" in body

    out = _post_json(f"http://127.0.0.1:{port}/pipeline/cancel", {"run_id": "abc"})
